"""
2025/11/28
다 학습된 모델을 불러와서 평가하는 것 같은데 분석해보자
"""
...
def eval_linear(args):
    utils.init_distributed_mode(args) #gpu 나누기
    print("\n".join("%s: %s" % (k, str(v)) for k, v in sorted(dict(vars(args)).items())))
    cudnn.benchmark = True #가장 적합한 알고리즘 찾기 

    if args.arch in vits.__dict__.keys():
        model = vits.__dict__[args.arch](patch_size = args.patch_size, num_classes=0)
        embed_dim = model.embed_dim * (args.n_last_blocks + int(args.avgpool_patchtokens)) #4 + 0
    model.cuda()
    model.eval()
    state_dict = torch.load(args.pretrained_weights, map_location="cpu")
    """
    state_dict는 모델의 전체 가중치를 블러오는 구조임
    모델 구조를 알고싶으면 print(model) 또는 
    ...
    linear_classifier = LinearClassifier(dim = embed_dim, num_labels = args.num_labels)
    ...
    optimizer = torch.optim.SGD(
        linear_classifier.parameters(), # backbone(불러온 가중치)는 유지하고, linear classifier만 학습한다
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser('ImageNet으로 linear classification test하기')
